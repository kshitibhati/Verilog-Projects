  ...
  always@(posedge clk)
   if (!rst) begin
      if (wr_enb)  
          rm[wr_addr] <= wr_data;
	   if (rd_enb)
	      temp_data <= rm[rd_addr];
   end
   
 always@(posedge clk)
   if (!rst && rd_data !== 0 && rd_data !== 8'dx) begin
      if (rd_data !== temp_data)
	      $display(" DATA MISMATCH ! RD_data = %d : %d = temp_data",rd_data,temp_data);
	  else
          $display(" SUCCESS ! RD_data = %d : %d = temp_data",rd_data,temp_data);
   end
